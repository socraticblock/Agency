...
- [036] `/start` accordion customizer: `CollapsibleSection` converted to controlled component with `isOpen`/`onToggle` props; `StartCustomizer` uses single `openSection: string | null` state for accordion behavior (only one section open at a time); smooth 300ms height transition with opacity fade; auto-scroll into view on open (`scrollIntoView` with `block: "nearest"`); chevron rotation 200ms ease-out; unique IDs for all 8 sections (content, photo, services, social, background, accent, font, addons); default opens "content" section.
...
## [036]
- **Context:** Multiple customizer sections could be open simultaneously, causing excessive scrolling and hiding the live preview from users.
- **Decision:** Converted `CollapsibleSection` to a controlled component accepting `isOpen` and `onToggle` props; implemented accordion behavior in `StartCustomizer` with single `openSection: string | null` state that ensures only one section is open at a time; added smooth 300ms height transition with opacity fade using `maxHeight` inline styles; implemented auto-scroll into view on open with `scrollIntoView({ behavior: "smooth", block: "nearest" })`; chevron rotates 0° → 180° with 200ms transition; assigned unique IDs to all 8 sections (content, photo, services, social, background, accent, font, addons); default opens "content" section on mount.
- **Impact:** Users maintain visual context of the live preview while editing; accordion pattern reduces cognitive load; smooth animations feel polished and modern; works identically on desktop and mobile bottom sheet.
